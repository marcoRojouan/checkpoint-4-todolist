...
const AdminCreatePrioprity = () => {
  ...
  const formSubmit: SubmitHandler<PriorityType> = async (data) => {
    try {
      ...
      toast.error("Erreur lors de l'ajout de la priorité");
    }
  };

  return (
    <main className="w-full max-w-6xl mx-auto p-6">
      <h2 className="text-2xl text-gray-800">Créer des Priorités</h2>
      <section>
        <form
        ...
        >
          <label className="text-xl" htmlFor="label">
            Choisir le label de la priorité
            <input
              type="text"
...
export default AdminCreatePrioprity;
