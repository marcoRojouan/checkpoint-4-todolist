import axios from "axios";
import { useEffect, useState } from "react";
import { toast } from "react-toastify";
import AdminThingElement from "../components/AdminThingElement";
import type { ThingType } from "../lib/definitions";

const AdminThingsList = () => {
  const [thingData, setThingData] = useState<ThingType[]>([]);

  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_URL}/thing/`,
        );
        setThingData(response.data);
      } catch (error) {
        toast.error(
          "Impossible de charger les données des recettes, veuillez essayer ultérieurement.",
        );
      }
    };
    fetchData();
  }, []);

  const handleDelete = async (thingId: number) => {
    try {
      await axios.delete(`${import.meta.env.VITE_API_URL}/thing/${thingId}`);

      setThingData((previousThingData) =>
        previousThingData.filter((thing) => thing.id !== thingId),
      );

      toast.success("Tâche supprimé avec succès");
    } catch (err) {
      toast.error(
        "Erreur lors de la suppression, veuillez réesayer plus tard.",
      );
    }
  };
  return (
    <main className="w-full max-w-6xl mx-auto p-6">
      <h2 className="text-2xl text-gray-800">Gérer les Tâches</h2>
      <section className="bg-white shadow-md rounded-lg overflow-hidden">
        <article className="bg-white divide-y divide-gray-200">
          {thingData?.map((element: ThingType) => (
            <AdminThingElement
              key={element.id}
              handleDelete={handleDelete}
              thingData={element}
            />
          ))}
        </article>
      </section>
    </main>
  );
};

export default AdminThingsList;
